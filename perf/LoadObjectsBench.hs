...
-- every object of an rsync-ed (or Erik) tree on local disk, what the rsync and
-- Erik fetchers do after downloading.
--
-- Usage:
--   cabal run load-objects-bench -- DIR RSYNC_URL [repeats] [--dir=PATH] [--no-urls] [--cpus=N] [--keep]
...
-- it. `--no-urls` loads it the way the Erik fetcher does, without restoring
-- URLs from paths.
--
...
-- process.
--
-- The configured CPU count is the number of capabilities, or `--cpus`: an
-- Erik fetcher starts with `+RTS -N1` and the configured count, and raises
-- the capabilities to that before loading objects.
--
...
import           RPKI.Config
import           RPKI.Domain
import           RPKI.Fetch.DirectoryTraverse (loadObjectsFromFS)
import           RPKI.Logging
...
main = do
    ...
    (rootPath, url, repeats) <- case args of
        ...
        _ -> error "Usage: load-objects-bench DIR RSYNC_URL [repeats] [--dir=PATH] [--no-urls] [--cpus=N] [--keep]"

    caps <- getNumCapabilities
    printf "loading %s as %s%s, capabilities: %d, caches under %s\n\n"
        rootPath (show url) (if noUrls then " (no URLs, as Erik)" else "" :: String) caps baseDir

    withLogger (newLogConfig ErrorL MainLog) $ \logger ->
        forM_ [1 .. repeats] $ \i ->
            bracket (mkBenchContext baseDir cpus logger) (cleanup keep . fst) $ \(_, appContext) ->
                runIteration i appContext noUrls rootPath url
  where
...
runIteration :: Int -> AppContext SqliteBackend -> Bool -> FilePath -> RsyncURL -> IO ()
runIteration i appContext noUrls rootPath url = do
    ...
    (result, vs) <- runValidatorIO (newScopes "load-objects-bench") $
        if noUrls
            then loadObjectsFromFS appContext worldVersion (\_ _ -> Nothing) rootPath
            else loadRsyncRepository appContext worldVersion url rootPath

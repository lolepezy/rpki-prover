...
{-
    ...
    'RPKI.Fetch.RelayPool'), not a recursive fan-out. Two things follow from
    ...
    doFetch relays =
        withDir indexDir $ \_ ->
            U.ifJustM (getIndex relays) $ \(relayUri, index@ErikIndex {..}) -> do
                ...
                cached <- DB.roTxT database $ \tx db ->
                            DB.existingErikPartitions tx db [ ref.hash | ref <- partitionList ]
                enqueue pool
                    [ (workHash w, w)
                    | ref <- partitionList
                ...
                -}
                if null failures
                    then DB.rwTxT database $ \tx db -> DB.saveErikIndex tx db relayUri fqdn index
                    else logWarn logger $
      ...
      where
        ...
        getIndex theRelays = withAnyRelay logger theRelays $ \relayUri -> do
            ...
            existing <- DB.roTxT database $ \tx db -> DB.getErikIndex tx db relayUri fqdn
            case existing of
        ...
        processWork scope relayUri = \case
          ...
          where
            ...
            expandCachedPartition partitionHash =
                DB.roTxT database (\tx db -> DB.getErikPartition tx db partitionHash) >>= \case
            ...
            expandPartition partitionHash ErikPartition {..} = do
                ...
                have <- DB.roTxT database $ \tx db ->
                            DB.existingHashes tx db [ ref.hash | ref <- inScope ]

                let (cachedRefs, missingRefs) =
                        List.partition (\ref -> ref.hash `Set.member` have) inScope

                cachedMfts <- DB.roTxT database $ \tx db ->
                                DB.getObjectsByHashes tx db [ ref.hash | ref <- cachedRefs ]
            ...
            childrenToFetch entries = do
                ...
                have <- DB.roTxT database $ \tx db ->
                            DB.existingHashes tx db [ e.hash | e <- mftChildren ]

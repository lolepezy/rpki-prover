...
-- implementation, but some crude approximation of it.
-- 
resolveIP :: HostName -> ServiceName -> IO (Either NetworkError [Either NetworkError AddrInfo])
resolveIP hostName serviceName = do     
    z <- try $ getAddrInfo (Just defaultHints { addrSocketType = Stream }) (Just hostName) (Just serviceName)
    case z of 
        Left (e :: SomeException) -> 
            pure $ Left $ NetworkError $ U.fmt e            

        Right [] -> 
            pure $ Left $ NetworkError [i|No IP addresses resolved for #{hostName}:#{serviceName}.|]

        Right addresses -> do 
            -- pick up only IPv4 and IPv6 and sort them IPv6 first, 
            -- AF_INET is smaller than AF_INET6
            let ipv6first = sortOn (Down . addrFamily) $ 
                            filter ((\f -> f == AF_INET || f == AF_INET6) . addrFamily) addresses
                    
            putStrLn $ "ipv6first = " <> show (map addrAddress ipv6first)                    

            Right . reverse <$> tryAddresses ipv6first
  where    

    tryAddresses addressesToConnect = do 
        tried <- newTVarIO []
        waitForAtLeastOne addressesToConnect tried (length addressesToConnect)

    waitForAtLeastOne [] tried totalSize = 
        atomically $ 
            readTVar tried >>= \case
                [] -> retry
                z  -> 
                    case [ x | x@(Right _) <- z ] of 
                        [] 
                            -- no single successful one
                            | length z == totalSize -> pure z 
                            -- there are still addresses to try
                            | otherwise             -> retry
                        -- at least one connection was successful
                        a  -> pure a
        
    waitForAtLeastOne (addr : adresses) tried totalSize =        
        withAsync (tryConnecting addr tried) $ \_ -> 
            waitForAtLeastOne adresses tried totalSize
             
    tryConnecting a@AddrInfo {..} tried = do 
        let resolutionDelay = 250_000
        case addrFamily of 
            -- give IPv6 connection some advantage
            AF_INET -> threadDelay resolutionDelay        
            _       -> pure ()
        ...
        let r = case z of 
                Left (e :: SomeException) -> let 
                        message = [i|Could not connect to #{addrAddress}: #{U.fmt e}|]
                    in Left $ NetworkError message
                Right _ -> 
                    Right a

        atomically $ modifyTVar' tried (r:)

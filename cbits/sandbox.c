 ...
 *
 * If either is set, everything else on the filesystem is denied, as are
...
static void rpki_prover_sandbox(void)
{
    ...
    struct ll_ruleset_attr attr = {
        ...
        .handled_access_net = abi >= 4 ? (LL_NET_BIND_TCP | LL_NET_CONNECT_TCP) : 0,
    ...
    if (add_rules(ruleset, rw, fs_rights) &&
        add_rules(ruleset, ro, LL_FS_READ_FILE | LL_FS_READ_DIR)) {

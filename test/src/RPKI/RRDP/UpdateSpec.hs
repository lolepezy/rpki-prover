...
testSnapshot = 
    HU.testCase "Should generate update snapshot action" $ do
        let repo = defaultRepo & typed .~ Just (newRrdpMeta (SessionId "whatever") (RrdpSerial 50))                    
...
testIntegrity = 
    HU.testCase "Should fall-back to snapshot in case of delta mutations" $ do
        ...
        let repo = defaultRepo & typed .~ Just (RrdpMeta sessionId serial13 (RrdpIntegrity previousDeltas) Nothing)
